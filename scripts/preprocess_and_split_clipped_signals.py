...
# Window-level shock detection. These shocks are treated as non-cutting transients.
WINDOW_S = 0.05
WINDOW_N = int(FS * WINDOW_S)
MAD_K = 8.0
EXPAND_S = 0.20
EXPAND_W = int(round(EXPAND_S / WINDOW_S))

# Lightweight frequency-domain cleaning. Keep tooth-passing and vibration bands,
# remove DC/drift and very-high-frequency acquisition noise.
HP_HZ = 20.0
LP_HZ = 10000.0
...
def detect_invalid_windows(arr: np.ndarray) -> tuple[np.ndarray, dict[str, float]]:
    ...
    if n_full == 0:
        return np.zeros(0, dtype=bool), {
            ...
            "invalid_window_count_expanded": 0,
        }
    ...
    if raw_count:
        ...
        for i in np.where(invalid)[0]:
            start = max(0, i - EXPAND_W)
            end = min(n_full, i + EXPAND_W + 1)
    ...
    return invalid, {
        ...
        "invalid_window_count_expanded": int(invalid.sum()),
    }
...
def preprocess(df: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, float]]:
    ...
    out = cleaned_df.copy()
    # Rebuild time from zero after removing invalid windows so each preprocessed
    # signal is continuous for feature extraction/CNN input.
...
def main():
    ...
    output_dir = input_dir.parent / "预处理并切分后实验数据"
        ...
        summary_rows.append(
            {
                ...
                "扩展后异常窗口数": info["invalid_window_count_expanded"],
                "切分段数": n_seg,
                "状态": status,
            }
        )
    ...
    (output_dir / "preprocess_config.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
